...
get_usergroups_list <- function(token, offset = 0) {
  result <- get_usergroups_list_single(token, offset)
...
#' single request for getting usergroups
#'
#' @param token login token generated by \code{\link{get_login_token}}
#' @param offset defailt 0, integer, where to start fetching usergroups
#'
...
get_usergroups_list_single <- function(token, offset = 0) {

  url <- paste0(token$url, "/api/user-groups/v1")
